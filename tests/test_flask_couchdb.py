...
class BlogPost(flask.ext.couchdb.Document):
    ...
    title = flask.ext.couchdb.TextField()
    text = flask.ext.couchdb.TextField()
    author = flask.ext.couchdb.TextField()
    tags = flask.ext.couchdb.ListField(flask.ext.couchdb.TextField())
    created = flask.ext.couchdb.DateTimeField(default=datetime.now)
...
class TestFlaskCouchDB(testutil.TempDatabaseMixin, unittest.TestCase):
    def setUp(self):
        super(TestFlaskCouchDB, self).setUp()
